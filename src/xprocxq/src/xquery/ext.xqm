...
(: -------------------------------------------------------------------------- :)
let $serialize-option := util:declare-option('exist:serialize','indent=no')
,$xsl-uri := xs:string(data($options/p:with-option[@name = 'xsl-uri']/@select))
return
	
		processing-instruction {'xml-stylesheet'} {concat('href="',  $xsl-uri, '" type="text/xsl"')}
