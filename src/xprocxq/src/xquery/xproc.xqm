...
(: -------------------------------------------------------------------------- :)
 ...
 return
    ...
    let $result :=
        (u:call(
		$xproc:parse-and-eval,
		<p:declare-step xproc:defaultname="{$defaultname}" >
              ...
              if ($child instance of element())
                then
            		if ($child intersect $select) then
                          (u:call(
				$xproc:parse-and-eval,
				<p:declare-step name="{$defaultname}" xproc:defaultname="{$defaultname}" >{$currentstep/node()}</p:declare-step>,
				(),
				$child,
				(),
				$outputs)/.)[last()]/node()
...
(: -------------------------------------------------------------------------- :)
    ...
    return
         ...
         util:catch('*',
             (u:call(
		$xproc:parse-and-eval,
		<p:declare-step name="{$defaultname}" xproc:defaultname="{$defaultname}" >{$currentstep/p:group}</p:declare-step>,
		$namespaces,
		$v,
		(),
		$outputs)/.)[last()]/node(),
             (u:call(
		$xproc:parse-and-eval,
		<p:declare-step name="{$defaultname}" xproc:defaultname="{$defaultname}" >{$currentstep/p:catch}</p:declare-step>,
		$namespaces,
		$v,
		(),
		$outputs)/.)[last()]/node())
...
(: ----------------------------------------------------------------------------- :)
    ...
    return
        if ($when) then
            (u:call(
		$xproc:parse-and-eval,
		<p:declare-step name="{$defaultname}" xproc:defaultname="{$defaultname}" >{$when/node()}</p:declare-step>,
		$namespaces,
		$v,
		(),
		$outputs)/.)[last()]/node()
            ...
            (u:call(
		$xproc:parse-and-eval,
		<p:declare-step name="{$defaultname}" xproc:defaultname="{$defaultname}" >{$otherwise/node()}</p:declare-step>,
		$namespaces,
		$v,
		(),
		$outputs)/.)[last()]/node()
            ...
            return
                ...
                if($currentstep/p:output[@primary='true']) then
                      <xproc:output step="{$step}"
						...
						$options,
						$currentstep,
						($outputs,$primaryinput,$secondaryinput))
                                           else
                                             u:call(
						u:xquery($stepfunc),
						$primary,
